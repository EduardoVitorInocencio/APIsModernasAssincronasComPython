...
async def get_current_user(db:Session = Depends(get_session), token: str = Depends(oauth2_schema)) -> UsuarioModel:
    ...
    credential_exeception: HTTPException = HTTPException(
        status_code = status.HTTP_401_UNAUTHORIZED,
        detail= 'Não foi possível autenticar a credencial',
        headers= {"WWW-Authenticate": "Bearer"},  
    )
    
    #Decotificação do nosso token para obter os payloads que são os dados desejados
    try:
        payload = jwt.decode(
            token,
            settings.JWT_SECRET,
    ...
    async with db as session: #Consulta do banco de dados
